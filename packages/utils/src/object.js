import { isReactive, isRef } from '@effectlzx/core'
import { noop, type } from './base'
...
function isPlainObject(value) {
  ...
  const observeClassInstance = global.__effectlzx?.config.observeClassInstance
    ...
    if (typeof a !== 'object' || a === null) {
      ...
      if (isPlainObject(a)) {
        ...
        for (let i = 0; i < length; i++) {
          const key = keys[i]
          curPath += `.${key}`
          clone[key] = deepDiffAndCloneA(
            a[key],
            sameClass ? b[key] : undefined,
            currentDiff,
            !(sameClass && hasOwn(b, key))
          )
          curPath = lastPath
        }
        ...
        for (let i = 0; i < length; i++) {
          curPath += `[${i}]`
          clone[i] = deepDiffAndCloneA(
            a[i],
            sameClass ? b[i] : undefined,
            currentDiff,
            !(sameClass && i < b.length)
          )
          curPath = lastPath
        }
