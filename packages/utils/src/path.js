import { set } from '@effectlzx/core'
...
function endStack() {
  ...
  // 字符串栈直接拼接
  const result =
    curStack.type === 'string'
      ? '__effectlzx_str_' + curStack.value.join('')
      : curStack.value
...
function outPutByPath(context, path, isSimple, transfer) {
  ...
  for (let index = 0; index < len; index++) {
    ...
    if (result) {
      ...
      } else if (/^__effectlzx_str_/.test(item)) {
        // 字符串一定会被[]包裹，一定在子数组中
        result = item.replace('__effectlzx_str_', '')
      } else if (/^\d+$/.test(item)) {
        // 数字一定会被[]包裹，一定在子数组中
        result = +item
      } else if (item === '+') {
...
export { getByPath, setByPath, doGetByPath, getFirstKey, aIsSubPathOfB }
