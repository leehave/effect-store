import Mpx, { computed, effectScope, reactive } from '@effectlzx/core'
import { getByPath, proxy, warn } from '@effectlzx/utils'
  ...
  Object.keys(deps).forEach((key) => {
    ...
    mergeProps.forEach((prop) => {
      if (module[prop] && key in module[prop]) {
        warn(
          `Deps's name [${key}] conflicts with ${prop}'s key in current options.`
        )
      } else {
...
class Store {
  ...
  registerModule(module) {
    ...
    if (module.getters) {
      reactiveModule.getters = transformGetters(
        module.getters,
        reactiveModule,
        this
      )
    ...
    if (module.mutations) {
      reactiveModule.mutations = transformMutations(
        module.mutations,
        reactiveModule,
        this
      )
    ...
    if (module.actions) {
      reactiveModule.actions = transformActions(
        module.actions,
        reactiveModule,
        this
      )
